...
function Index(connection, indexId, columns){
  ...
  this.findIn = this._exec.bind(this, 'IN');
  ...
  this.updateAndGet = this._exec.bind(this, 'UPDATE_AND_GET');
  this.deleteAndGet = this._exec.bind(this, 'DELETE_AND_GET');
  this.incrementAndGet = this._exec.bind(this, 'INCREMENT_AND_GET');
  this.decrementAndGet = this._exec.bind(this, 'DECREMENT_AND_GET');
}
...
Index.prototype._exec = function(order, values, options, cb){
  ...
  var self = this;
  var req = [this.indexId, operator];

  req = req.concat(_parseOptions(order, values, options));
  ...
  var next = function(err, res){
    ...
    , function(err, data){
      cb(err, err ? null : assignKey(columns, resultsToArray(data)));
    });
  }
...
function _parseOptions(order, values, options){
  ...
  if(order === 'IN' || options.in) {
    addReq = addReq.concat([1, '', limit, offset, orders.IN, 0, values.length, values.join(SEP)]);
  } else {
    addReq = addReq.concat([values.length, values.join(SEP), limit, offset]);
  }

  addReq = addReq.concat(_addFilterRequest(options), _addModRequest(order, values));
  
  return addReq;
}

function _addModRequest(order, values){
  return [];
}
