...
HandlerSocket.prototype._auth = function(cb){
  ...
  if(!this.enableAuthentication){
    ...
    this.connection.send([order.AUTH, this.auth.type, this.auth.key], next);
  }
}
...
HandlerSocket.prototype.openIndex = function(dbName, tableName, indexName, columns, fcolumns, cb){
  ...
  var req = [order.OPEN_INDEX, indexId, dbName, tableName, indexName, columns.join(',')];
...
Index.prototype.findIn = function(values, options, cb){
  options.inClause = [order.IN, 0, values.length, values.join(SEP)];
...
function crateFilterRequest(options){

  if(options.filter){
    return [order.FILTER, options.filter[1], options.filter[0], options.filter[2]];
  }

  if(options.while){
    return [order.WHILE, options.while[1], options.while[0], options.while[2]];
  }
