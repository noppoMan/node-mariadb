...
Index.prototype.findIn = function(values, options, cb){
  options.inClause = [cmds.IN, 0, values.length, values.join(SEP)];
...
function crateFilterRequest(options){
  ...
  function parseOptions(){
    return [options.filter[1], options.filter[0], options.filter[2]];
  }

  if(options.filter){
    return [cmds.FILTER].concat(parseOptions());
  }
  if(options.whileFilter){
    return [cmds.WHILE_FILTER].concat(parseOptions());
  }
